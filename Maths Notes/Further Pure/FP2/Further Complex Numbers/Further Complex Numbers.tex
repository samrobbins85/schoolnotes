...
\section{Absolute square}
When squaring $|z|$, where $z=x+iy$, first find $|z|$, which is $\sqrt{x^2+y^2}$, then square it to get $x^2+y^2$.
...
\end{itemize}
When doing translations, the input is $z=x+iy$ and the output is $w=u+iv$, unless otherwise stated.
\end{document}